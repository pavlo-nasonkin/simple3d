...
- [ ] **1.1. Объявить `ILightingModel`.** Самостоятельный интерфейс,
...
- [ ] **1.2. Маркеры в `defaultColorLight.fs`.** Заменить inline-лайтинг
...
- [ ] **1.3. `Material3D` владеет `unique_ptr<ILightingModel>`.**
  ```cpp
...
- [ ] **1.4. `Filter3D::Bind` остаётся без изменений.** Это
...
- [ ] **2.1. `PhongLightingModel`** — миграция существующей логики.
...
- [ ] **2.2. `UnlitLightingModel`** — для источников света и UI-материалов.
...
- [ ] **2.3. Миграция потребителей.** Везде, где сейчас создаётся
...
- [ ] **2.4. Чистка `defaultColorLight.fs`.** После 1.2 + 2.1 файл должен
...
- **5.9 (Расширяемый `Vertex`)** становится критичным на этапе 5: skybox
  использует свой layout без UV, PBR может захотеть `vec3 color` per-
  vertex. Без 5.9 каждый новый layout ломает существующий VAO.
