...
- [x] **3.1. Новые `FilterSlot`.** Добавить в `Filter3D::FilterSlot`:
...
- [x] **3.2. Фильтры под `float`-результат.** Сейчас все фильтры возвращают
  ...
  - Реализовано через swizzle `.r` в `InjectFilters`. NB: `ResultType::FLOAT`
    добавлен в enum, но фактически не используется (swizzle-вариант) — можно
    убрать как мёртвый код.

- [x] **3.3. Маппинг Assimp-типов на новые слоты.** В
...
- [x] **4.1. PBR-математика.** `GetLightingCode()` возвращает классический
  Cook-Torrance BRDF:
...
- [x] **4.2. Direct lights only.** На первом этапе считаем только один
  point light из `Scene3D` (то же, что у Phong сейчас). IBL — следующая
  задача.

- [x] **4.3. Тестовая сцена.** В `main.cpp` добавить флаг (или второй
...
- [ ] **4.4. Directional light (солнце) для PBR.** Сейчас `PBRLightingModel`
  использует тот же point light (`light.position`), что и Phong. Завести
  отдельный directional-источник и использовать его как основной прямой
  свет в PBR. Phong не трогаем — он продолжает работать на старом point light.

  - **4.4.1. Новый источник в `Scene3D`.** Добавить directional light как
    отдельную сущность (не ломая существующие `_lightPosition/_lightAmbient/...`):
    ```cpp
    ...
    ```
    плюс геттеры/сеттеры по аналогии с текущими.
    - Файлы: `src/Scene3D.h`, `src/Scene3D.cpp`.

  - **4.4.2. `PBRLightingModel` читает directional light.**
    - `GetDeclarations()`: добавить
      ```glsl
      struct DirLight { vec3 direction; vec3 color; };
      uniform DirLight dirLight;
      ```
    - `GetLightingCode()`: заменить вычисление `L`/`radiance` на directional:
      ```glsl
      vec3 L = normalize(-dirLight.direction); // без attenuation
      vec3 radiance = dirLight.color;
      ```
      Остальной Cook-Torrance (D/F/G, `kD`, ambient, tone map) — без изменений.
    - `Bind()`: выставить `dirLight.direction` и `dirLight.color`
      (`= color * intensity`) из `ctx.scene3D`. Убрать чтение `light.position`,
      если оно больше не используется в PBR-коде.
    - **Соглашение о направлении:** хранить вектор «куда светит», в шейдере
      инвертировать (`-direction`) для получения вектора «на источник».
    - Файл: `src/materials/lighting/PBRLightingModel.h/.cpp`.

  - **4.4.3. Настройка в сцене.** В `main.cpp`/инициализации сцены задать
    направление и тон солнца (`setDirLightDirection(...)`, `setDirLightColor(...)`).
    - Файл: `main.cpp`.

  - **Замечания:**
    - Directional не имеет позиции и затухания (`1/d²` не применяется) — это
      и отличает его от point light.
    - Яркость регулируется `intensity` (из-за `/π` прямой свет иначе тусклый),
      а не задиранием каналов цвета — тон остаётся нормированным.
    - Ambient остаётся плоским (`light.ambient ≈ 0.03`) до появления IBL (Этап 5).
    - Это уточняет/заменяет п. 4.2: «direct lights only» теперь = один
      directional источник вместо point.
...
- [x] **5.1. `SceneEnvironment` — отдельная сущность сцены.**
  ```cpp
...
- [x] **5.2. Класс `TextureCube`.** Сейчас есть только `Texture2D`.
  - Файлы: `src/resources/TextureCube.h/.cpp`.
  - Сделан RAII-классом (move-only, как `GLBuffer`/`GLVertexArray`), с
    `Bind(unit)`/`Unbind()` и фабрикой `CreateEmpty(...)` для пустого cubemap'а
    под render-target/IBL.

- [x] **5.3. Загрузчик HDR equirectangular → cubemap.** Один HDR-файл
  `.hdr` → cubemap через однократный render-pass.
  - Файл: `src/resources/HDRLoader.cpp` (`HDRLoader::EquirectFileToCubemap`).
  - Загрузчик самодостаточный (свой RGBE-парсер `32-bit_rle_rgbe`), без
    внешних зависимостей: SOIL — prebuilt DLL и не экспортирует stb-символы,
    а его `stb_image.h` сильно кастомизирован. **`.exr` не поддерживается**
    (нужен tinyexr — отдельной задачей при необходимости).
  - Бейк: equirect → `GL_RGB16F` 2D-текстура → рендер юнит-куба 6× с 90°-проекцией
    в грани cubemap'а через FBO; GL-состояние (viewport/FBO/cull) сохраняется и
    восстанавливается.

- [x] **5.4. Прекомпиляция irradiance + prefiltered specular + BRDF LUT.**
  ...
  - `IBLBaker::BakeIrradiance(env, 32)` — свёртка по полусфере (diffuse IBL).
  - `IBLBaker::BakePrefiltered(env, 128, 5)` — GGX importance sampling, roughness
    по мип-уровням; перед бейком на env генерируются мипы (для качества выборки).
  - `IBLBaker::BakeBRDFLUT(512)` — RG16F LUT (scale/bias для F0), независим от окружения.
  - Шейдеры — inline в `IBLBaker.cpp`; рендер в грани cubemap'а / quad через FBO,
    GL-состояние сохраняется через `GLStateGuard`.
  - **Ещё не подключено в `Scene3D`/`PBRLightingModel`** — это пункт 5.5.

- [x] **5.5. `PBRLightingModel` использует IBL.** В `Bind()` биндит
  ...
  `light.ambient`.
  - `SceneEnvironment` получил `Set/IsValid/Irradiance/PrefilteredSpec/BrdfLUT`,
    `Scene3D::SetEnvironment(...)` + `GetEnvironment()`.
  - Шейдер: `uHasIBL`, `irradianceMap`/`prefilterMap`/`brdfLUT`,
    `FresnelSchlickRoughness`; ambient = `kD*irradiance*albedo + prefiltered*(F*brdf.x+brdf.y)`,
    с fallback на плоский `ambientLight`, когда окружения нет.
  - Бейк карт и `SetEnvironment` — в `main.cpp` сразу после загрузки cubemap.

- [x] **5.6. Skybox.** Отрисовка cubemap'а как фона сцены — отдельный
  vertex+fragment шейдер. Это не часть `ILightingModel`, это отдельный
  pass в `Scene3D::Render`.
  - Файлы: `src/render/Skybox.h/.cpp`, `assets/shaders/skybox.vsh/.fsh`.
  - `Scene3D::SetSkybox(cubemap)` создаёт `Skybox`; рисуется последним с
    `glDepthFunc(GL_LEQUAL)` и трюком `gl_Position = pos.xyww` (depth = 1.0),
    view без translation. Пропускается в id-pass'е (`material != nullptr`).
    Линейный HDR из cubemap'а приводится тем же Reinhard+gamma, что и PBR.
...
## Этап 7. Cast shadows (directional shadow mapping)

Цель: объекты (машина, ящик) отбрасывают тень на пол от **directional sun**
(`dirLight`, заведён на Этапе 4.4). Точечный/Phong-свет и omni-тени (cube shadow
map) — вне scope, фокус на одном направленном источнике для PBR.

### Что уже есть и переиспользуется
- **FBO + offscreen render** — паттерн отработан в `HDRLoader`/`IBLBaker`
  (`GLStateGuard`, save/restore viewport+FBO).
- **Override-материал на проход** — `Pivot3D::Render(ctx, material)` уже умеет
  рендерить всю иерархию одним материалом (так делает id-pass `ObjectSelector`
  с `ObjectIdMaterial`). Depth-pass — такой же приём.
- **Мировая позиция фрагмента** — `FragPos` (world) уже идёт во фрагментный
  шейдер, значит `fragPosLightSpace` можно посчитать в FS без правки vertex-шейдеров.

### Подэтапы

- [x] **7.1. Класс `ShadowMap`.** Владеет depth-FBO + depth-текстурой
  (`GL_DEPTH_COMPONENT24`, `GL_NEAREST`, `GL_CLAMP_TO_BORDER` с border=1.0,
  чтобы вне карты не было тени). `Begin()` биндит FBO + ставит viewport в
  разрешение карты (по умолч. 2048²) и чистит depth; `End()` —
  восстанавливает предыдущий FBO/viewport. `glDrawBuffer/glReadBuffer = NONE`
  (depth-only completeness).
  - Файлы: `src/render/ShadowMap.h/.cpp`. **Сделано.**
  - Флаги узла: в `Pivot3D` добавлены `_castShadows`/`_receiveShadows`
    (default true) + геттеры/сеттеры. `castShadows` будет проверяться в
    depth-pass (7.4), `receiveShadows` — пробрасываться в PBR (7.5).

- [x] **7.2. Light-space матрица.** `ShadowMap::ComputeLightSpaceMatrix(dir, center, radius)`:
  ortho-куб `2*radius` + `lookAt` из `center - dir*radius` вдоль `dirLight.direction`,
  глубина `[0, 2*radius]`, защита `up` при свете строго вниз. **Сделано.**
  Для directional света — ортографическая проекция + `lookAt` вдоль `dirLight.direction`:
  ```cpp
  glm::mat4 lightView = glm::lookAt(-dir * d, vec3(0), up);
  glm::mat4 lightProj = glm::ortho(-S, S, -S, S, near, far);
  lightSpaceMatrix = lightProj * lightView;
  ```
  Первый вариант — фиксированный куб вокруг начала координат (S, d, near/far —
  параметры). Подгонка под фрустум камеры / CSM — отдельная будущая задача.
  - Метод `ShadowMap::ComputeLightSpace(const glm::vec3& dir, ...)`.

- [x] **7.3. Depth-материал и проход.** `DepthMaterial : MaterialBase`
  (тонкий подкласс) + шейдеры `depth.vsh`/`depth.fsh`. **Вариант 1**: depth.vsh
  использует обычные `model/view/projection`, которые базовый `MaterialBase::Bind`
  и так выставляет; в shadow-pass `view/projection` = light-матрицы через
  `RenderContext` (без правки `MaterialBase`). fsh пустой (пишем только depth).
  vsh берёт только `location 0` (position) — лишние атрибуты VAO игнорируются. **Сделано.**
  - **Skinned-меши (TODO)**: нужен скелетный вариант depth-шейдера (как `shader_skin.vsh`),
    иначе анимированные модели отбросят тень в bind-позе. Первый шаг — статика.
  - Файлы: `src/materials/DepthMaterial.h/.cpp`,
    `assets/shaders/depth.vsh`, `assets/shaders/depth.fsh`.

- [x] **7.4. Интеграция в `Scene3D::Render`.** **Сделано.** `Scene3D::EnableShadows(size, radius)`
  создаёт `ShadowMap` + `DepthMaterial`. В `Render` (только при `material==nullptr`):
  shadow-pass с `shadowCtx.view=I`, `projection=lightSpace`, `shadowPass=true`, затем
  основной проход получает `lightSpaceMatrix`/`shadowMap`/`hasShadows` в `RenderContext`.
  `_castShadows` проверяется в `Pivot3D::Render` и `Mesh::Render` (пропуск поддерева в shadow-pass).
  Порядок кадра:
  1. shadow-pass: `_shadowMap->Begin()`; `RenderContext shadowCtx` с
     `view=lightView`, `projection=lightProj`; `Pivot3D::Render(shadowCtx, depthMaterial)`;
     `_shadowMap->End()`.
  2. основной pass: как сейчас, но в `RenderContext` добавляются
     `glm::mat4 lightSpaceMatrix` и `GLuint shadowMapTex` (+ флаг `hasShadows`).
  - `Scene3D` владеет `std::shared_ptr<ShadowMap>` и `DepthMaterial`.
  - Поля `lightSpaceMatrix`/`shadowMapTex`/`hasShadows` добавляются в
    `src/render/RenderContext.h`.
  - Тень считается только в обычном проходе (как и skybox: `material == nullptr`),
    в id-pass'е пропускается.

- [x] **7.5. `PBRLightingModel` сэмплит shadow map.** **Сделано.** Тень множит только
  `Lo` (direct), IBL/ambient не трогает. PCF 3×3 + slope-scaled bias, `proj.z>1` отсекается.
  `receiveShadows` пробрасывается из `Mesh` через `Material3D::Bind` → `RenderContext` →
  `uHasShadows = hasShadows && receiveShadows`. Shadow map на `firstTextureUnit+3`,
  `GetTextureUnitCount()→4`.
  - Декларации: `uniform sampler2D shadowMap; uniform mat4 uLightSpaceMatrix; uniform bool uHasShadows;`
  - В коде: `vec4 fpLS = uLightSpaceMatrix * vec4(FragPos,1.0);` → NDC→[0,1],
    PCF 3×3 по `textureSize`, сравнение с bias.
  - **Тень множит только прямой (direct) вклад**: `Lo *= (1.0 - shadow);`
    IBL/ambient тенью **не** гасится (это непрямой свет — иначе тени станут
    чёрными и нефизичными).
  - `Bind()`: shadow map биндится на следующий свободный юнит после IBL
    (`firstTextureUnit + 3`), `GetTextureUnitCount()` → 4. Выставляются
    `uLightSpaceMatrix`/`uHasShadows` из `ctx`.
  - Bias: `max(0.0008 * (1.0 - NdotL), 0.0005)` + опц. normal-offset, чтобы
    убрать shadow acne; `GL_CLAMP_TO_BORDER` border=1 убирает тень за картой.

- [ ] **7.6. Тюнинг (по необходимости).**
  - Front-face culling в depth-pass (или polygon offset) против peter-panning/acne.
  - Подбор `S`/`near`/`far` ortho-фрустума под сцену; разрешение карты.
  - Позже: cascaded shadow maps (CSM) для больших открытых сцен,
    skinned depth-вариант, omni-тени для point light.

### Антипаттерны, которых избегаем
- **Гасить тенью ambient/IBL.** Тень — только для direct dirLight; непрямой
  свет окружения по определению приходит «в обход» прямого луча.
- **Своя FBO-возня в каждом проходе.** Save/restore состояния — через общий
  `GLStateGuard` (как в IBL/HDR), а не ручным `glGetIntegerv` в пяти местах.
- **Захардкоженный текстур-юнит shadow map.** Юнит выделяется через
  `firstTextureUnit + GetTextureUnitCount()`-аккаунтинг, как у IBL-карт.

---

## Этап 8. Редактор (ImGui)

Цель: импорт моделей → сохранение как **префабы** → расстановка префабов на
сцене → сохранение/загрузка **сцены** (на старте можно загрузить сцену).

**Стек выбран:** Dear ImGui (ветка docking) поверх существующего GLFW-окна +
`nativefiledialog-extended` (диалоги открытия/сохранения) + `nlohmann/json`
(сериализация). GLFW остаётся как окно/ввод; ImGui — immediate-mode UI поверх
GL-контекста. Зависимости — через vcpkg.

### Предпосылки
- **Этап 6 (Geometry split)** — фактическая предпосылка: префаб, инстанцируемый
  многократно, требует разделения «геометрия (шарится) + узел-инстанс (свой
  transform)». Без него два инстанса префаба встанут в одну точку (см.
  антипаттерн «расшарить `shared_ptr<Mesh>`»). Редактор можно начать и до 6,
  но множественные инстансы одного префаба до 6 будут некорректны.
- Пикинг уже есть (`ObjectSelector`, id-pass) — переиспользуется для выделения.
- FBO-паттерн уже есть (`ShadowMap`/`IBLBaker`) — переиспользуется для вьюпорта.

### Подэтапы

- [ ] **8.1. Интеграция ImGui.** Подключить `imgui[docking-experimental,
  glfw-binding,opengl3-binding]`. Init после `glewInit`, шатдаун перед
  `glfwTerminate`. В главном цикле: `NewFrame` → построение UI → `Render` +
  `ImGui_ImplOpenGL3_RenderDrawData`. Включить docking + (опц.) viewports.
  - Файлы: `src/editor/EditorUI.h/.cpp`, правки `main.cpp`, `CMakeLists.txt`.
  - NB: ImGui должен получать ввод раньше камеры — пробрасывать
    `io.WantCaptureMouse/Keyboard`, чтобы клики по панелям не крутили камеру.

- [ ] **8.2. Вьюпорт как панель (render-to-texture).** Рендер сцены в offscreen
  FBO (color `GL_RGBA8` + depth), показ через `ImGui::Image` в докнутом окне
  «Viewport». Ресайз FBO под размер панели. Камера использует размер вьюпорта,
  а не окна (поправить `SetScreenWidth/Height`, центр для FPS-камеры).
  - Файлы: `src/render/Framebuffer.h/.cpp` (обобщить FBO), `src/editor/ViewportPanel`.

- [ ] **8.3. Панель иерархии + инспектор.** Дерево `Scene3D` (обход `Children()`),
  выбор узла (синхрон с `ObjectSelector`). Инспектор: position/rotation/scale,
  имя, флаги `castShadows/receiveShadows`, параметры материала (roughnessScale,
  baseColor). Правки пишутся прямо в `Pivot3D`/`Material3D`.
  - **Инспектор материала (расширение)**: список фильтров материала с
    возможностью добавлять/удалять/переупорядочивать, менять слот/blend и
    выбирать текстуру — те же поля, что сериализуются в 8.5a (один источник
    истины: `Filter3D::Serialize` + `FilterFactory`). Требует, чтобы фильтры
    были «осматриваемыми» (тип + параметры доступны для UI).
  - Файлы: `src/editor/HierarchyPanel`, `src/editor/InspectorPanel`.

- [ ] **8.4. Импорт моделей.** Кнопка «Import» → NFD-диалог (`.fbx/.obj/.gltf…`)
  → `ExternalModel(path)` → добавление в сцену. Уже работает резолвер текстур
  по соглашению Megascans (Этап 3).
  - Файл: `src/editor/AssetBrowserPanel` (+ NFD).

- [ ] **8.5. Формат префаба (JSON).** Префаб = сериализованное поддерево узлов:
  ссылка на источник геометрии (путь к модели/тип примитива), **материал**
  (см. ниже), локальный transform, дочерние узлы, флаги теней.
  - `Prefab::Save(path)` / `Prefab::Load(path)` → инстанс поддерева `Pivot3D`.
  - Файлы: `src/scene/Prefab.h/.cpp`, `src/scene/Serialization.h/.cpp`.

- [ ] **8.5a. Сериализация материала (по полям и фильтрам).** Материал
  сохраняется **не ссылкой**, а своей структурой, чтобы его можно было собирать/
  править вручную (будущий инспектор материалов, п. 8.3):
  - поля материала: lighting-модель (`Phong`/`PBR`/`Unlit`), `roughnessScale`,
    `cullFace`, базовый цвет и т.п.;
  - **список фильтров** по порядку, каждый — `{ type, slot, blendMode, params }`,
    где `params` зависит от типа: `TextureMapFilter`/`NormalMapFilter` → путь к
    текстуре (+sRGB-флаг при появлении), `ColorFilter` → цвет, и т.д.
  - **Реестр фильтров (factory by type-string)**: для десериализации нужен
    `FilterFactory::Create(typeName, paramsJson)` — иначе по строке типа не
    построить нужный подкласс `Filter3D`. Каждый `Filter3D` обязан уметь
    `Serialize()`/знать свой `type`.
  - Текстуры **внутри** фильтров — по путям (относительным к корню ассетов),
    не по бинарю.
  - Файлы: `src/materials/MaterialSerializer.*`, `src/materials/filters/FilterFactory.*`,
    методы `Serialize`/`type` в `Filter3D` и подклассах.
  - Это прямой задел под **инспектор материалов**: добавление/удаление фильтров,
    смена слота/blend, выбор текстуры — те же поля, что в JSON.

- [ ] **8.6. Формат сцены (save/load).** Сцена = список инстансов префабов
  (ссылка на префаб + world-transform + переопределения), плюс параметры
  окружения: directional light (dir/color/intensity), ambient, HDR-окружение
  (путь к `.hdr`), shadow-параметры (radius/strength), камера.
  - `Scene3D::Save(path)` / `Scene3D::Load(path)`.
  - Файлы: `src/scene/SceneSerializer.h/.cpp`, правки `Scene3D`.

- [ ] **8.7. Загрузка сцены на старте.** Аргумент командной строки или
  «последняя сцена» из конфигурации; если задана — грузим вместо хардкода в
  `main.cpp`. Меню `File → New/Open/Save/Save As`.

- [ ] **8.8. Гизмо трансформации (опц.).** `ImGuizmo` (translate/rotate/scale)
  во вьюпорте поверх выбранного узла. Удобно, но не блокер для MVP.

### Замечания / решения
- **Виджеты не управляют сценой напрямую через глобалы.** Редактор работает с
  выбранным узлом через `ObjectSelector`, изменения — через сеттеры
  `Pivot3D`/`Material3D` (тот же контракт, что и в рантайме).
- **Ввод:** при `io.WantCaptureMouse` камера/пикинг игнорируют мышь — иначе
  драг по UI крутит камеру. Это первое, что ломается без проверки.
- **Сериализация по ссылкам.** Пути к моделям/текстурам/HDR — относительные к
  корню ассетов, чтобы сцены/префабы были переносимы.
- **MVP без Этапа 6** возможен (один инстанс на префаб), но множественные
  инстансы одного префаба требуют 6 (geometry sharing).
